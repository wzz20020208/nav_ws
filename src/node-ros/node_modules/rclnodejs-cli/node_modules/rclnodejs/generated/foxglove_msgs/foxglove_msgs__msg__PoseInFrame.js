...
const PoseInFrameRefStruct = StructType({
  timestamp: TimeWrapper.refObjectType,
  frame_id: primitiveTypes.string,
  pose: PoseWrapper.refObjectType,
});
const PoseInFrameRefArray = ArrayType(PoseInFrameRefStruct);
const PoseInFrameRefStructArray = StructType({
  data: PoseInFrameRefArray,
  size: ref.types.size_t,
  capacity: ref.types.size_t
});
// Define the wrapper class.
class PoseInFrameWrapper {
  constructor(other) {
    this._wrapperFields = {};
    if (typeof other === 'object' && other._refObject) {
      this._refObject = new PoseInFrameRefStruct(other._refObject.toObject());
  ...
  _initMembers() {
    this._refObject = new PoseInFrameRefStruct();
  ...
  static createFromRefObject(refObject) {
    let self = new PoseInFrameWrapper();
    self.copyRefObject(refObject);
    return self;
  }
  static createArray() {
    return new PoseInFrameArrayWrapper;
  }
  static get ArrayType() {
    return PoseInFrameArrayWrapper;
  }
  static get refObjectArrayType() {
    return PoseInFrameRefStructArray
  }
  static get refObjectType() {
    return PoseInFrameRefStruct;
  }
  toRawROS() {
    this.freeze(true);
  ...
  freeze(own = false, checkConsistency = false) {
    if (checkConsistency) {
      if (!this._frame_idIntialized) {
        throw new TypeError('Invalid argument: frame_id in PoseInFrame');
      }
    }
  ...
  static destoryRawROS(msg) {
    PoseInFrameWrapper.freeStruct(msg.refObject);
  }
  static type() {
    return {pkgName: 'foxglove_msgs', subFolder: 'msg', interfaceName: 'PoseInFrame'};
  }
  static isPrimitive() {
    return false;
  }
  ...
  copyRefObject(refObject) {
    this._refObject = new PoseInFrameRefStruct(refObject.toObject());
...
class PoseInFrameArrayWrapper {
  ...
  fill(values) {
    ...
    values.forEach((value, index) => {
      if (value instanceof PoseInFrameWrapper) {
        this._wrappers[index].copy(value);
      } else {
        this._wrappers[index] = new PoseInFrameWrapper(value);
      }
    });
  }
  ...
  set size(value) {
    if (typeof value != 'number') {
      throw new TypeError('Invalid argument: should provide a number to PoseInFrameArrayWrapper.size setter');
      return;
    }
  ...
  set capacity(value) {
    if (typeof value != 'number') {
      throw new TypeError('Invalid argument: should provide a number to PoseInFrameArrayWrapper.capacity setter');
    }
    return this._resize(value);
  }
  get refObject() {
  ...
  _resize(size) {
    ...
    this._refArray = new PoseInFrameRefArray(size);
    this._refObject = new PoseInFrameRefStructArray();
    this._refObject.size = size;
    this._refObject.capacity = size;
    this._wrappers = new Array();
    for (let i = 0; i < size; i++) {
      this._wrappers.push(new PoseInFrameWrapper());
    }
  }
  ...
  copy(other) {
    if (! (other instanceof PoseInFrameArrayWrapper)) {
      throw new TypeError('Invalid argument: should provide "PoseInFrameArrayWrapper".');
    }
  ...
  static freeArray(refObject) {
    ...
    for (let index = 0; index < refObject.size; index++) {
      PoseInFrameWrapper.freeStruct(refObjectArray[index]);
    }
  }
...
module.exports = PoseInFrameWrapper;
/*
