...
function validatePackageJson(typesDirectoryName, packageJson, typesVersions) {
    ...
    for (const depsKey of ["dependencies", "peerDependencies", "devDependencies"]) {
        const deps = packageJson[depsKey];
        if (deps && typeof deps === "object" && !Array.isArray(deps)) {
            errors.push(...checkDependencyVersions(typesDirectoryName, depsKey, deps));
        }
    }
    // typesVersions
    if (needsTypesVersions) {
...
function checkDependencyVersions(typesDirectoryName, depsKey, dependencies) {
    const errors = [];
    for (const dependencyName of Object.keys(dependencies)) {
        const version = dependencies[dependencyName];
        if (typeof version !== "string") {
            errors.push(`${typesDirectoryName}'s package.json has bad "${depsKey}": version for ${dependencyName} should be a string.`);
        }
        else if (version !== "workspace:." && !isValidRegistrySpec(version)) {
            errors.push(`${typesDirectoryName}'s package.json has bad "${depsKey}": version for ${dependencyName} (${JSON.stringify(version)}) must be a valid semver range, dist-tag, or "workspace:.".`);
        }
    }
    return errors;
}
