#include "nav2_custom_plugins_parallel/mppi_gpu_controller.hpp"
#include <limits>
#include <cmath>
#include <algorithm>
#include <random>
  ...
  nav2_core::GoalChecker *)
{
  ...
  // ══════════════════════════════════════════
  {
    ...
    if (state_ == ControllerState::HEADING_MISALIGN) {
      ...
      // ── 旋转方向安全检查 ──
      auto* hcm = costmap_ros_ ? costmap_ros_->getCostmap() : nullptr;
      if (hcm && hcm->getCharMap() && hcm->getSizeInCellsX() > 0) {
        const unsigned char* hd = hcm->getCharMap();
        int hw = hcm->getSizeInCellsX(), hh = hcm->getSizeInCellsY();
        float hr = hcm->getResolution(), hox = hcm->getOriginX(), hoy = hcm->getOriginY();

        auto checkRotSafe = [&](double sign) -> bool {
          // 用 dt × max_w 的旋转量, 采样 footprint 四角 + 中心
          double dt_check = 0.15;
          double theta_test = current_theta + sign * max_w_ * dt_check;
          double ct = std::cos(theta_test), st = std::sin(theta_test);
          double corners[5][2] = {
            { footprint_front_,  footprint_left_},
            { footprint_front_, -footprint_right_},
            {-footprint_back_,   footprint_left_},
            {-footprint_back_,  -footprint_right_},
            {0, 0}  // 中心
          };
          // 同时采样中间角度 (sweep)
          for (int si = 0; si <= 2 && si <= 2; ++si) {
            double alpha = si / 3.0;
            double ta = current_theta + sign * max_w_ * dt_check * alpha;
            double cta = std::cos(ta), sta = std::sin(ta);
            for (int ci = 0; ci < 5; ++ci) {
              double wx = current_x + corners[ci][0] * cta - corners[ci][1] * sta;
              double wy = current_y + corners[ci][0] * sta + corners[ci][1] * cta;
              int mx = static_cast<int>((wx - hox) / hr);
              int my = static_cast<int>((wy - hoy) / hr);
              if (mx >= 0 && mx < hw && my >= 0 && my < hh) {
                if (hd[my * hw + mx] >= 254) return false;
              }
            }
          }
          return true;
        ...
        bool fwd_safe = checkRotSafe(rot_sign);
        bool rev_safe = checkRotSafe(-rot_sign);

        if (!fwd_safe && rev_safe) {
          rot_sign = -rot_sign;
        } else if (!fwd_safe && !rev_safe) {
          rot_sign = 0.0;  // 两侧都堵, 停止旋转, 等下次状态机切换
        }
      }
...
PLUGINLIB_EXPORT_CLASS(nav2_custom_plugins_parallel::MPPIGPUController, nav2_core::Controller)
