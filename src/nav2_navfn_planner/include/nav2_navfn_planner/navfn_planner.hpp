...
protected:
  ...
  nav_msgs::msg::Path optimizeSE2Path(
    const nav_msgs::msg::Path & raw_path,
    const geometry_msgs::msg::PoseStamped & start,
    const geometry_msgs::msg::PoseStamped & goal);

  std::vector<SE2Pose> resamplePathForOptimization(
    const nav_msgs::msg::Path & path,
    double spacing);

  ESDFGrid buildESDFGrid();

  double queryESDF(const ESDFGrid & esdf, double wx, double wy) const;
  ...
  double computeFootprintObstacleCost(
  const SE2Pose & pose,
  const ESDFGrid & esdf) const;

  void assignContinuousYaw(
  std::vector<SE2Pose> & poses,
  const ESDFGrid & esdf,
  const geometry_msgs::msg::PoseStamped & start,
  const geometry_msgs::msg::PoseStamped & goal) const;

  bool validateOptimizedPath(
  const std::vector<SE2Pose> & poses,
  const ESDFGrid & esdf) const;

  void setPathOrientationsFromTangent(nav_msgs::msg::Path & path) const;

  double wrapAngle(double a) const;

  double angleDiff(double a, double b) const;

  bool getTemporalYawReference(const SE2Pose & pose, double & yaw) const;
  ...
  bool enable_se2_optimization_{true};
  double se2_resample_resolution_{0.10};
  double robot_length_{0.40};
  double robot_width_{0.60};
  double robot_safe_distance_{0.03};
  double se2_w_obs_{80.0};
  double se2_w_ref_{1.0};
  double se2_w_smooth_{8.0};
  double se2_w_yaw_smooth_{1.0};
  double se2_w_align_{0.4};
  double se2_w_tangent_{6.0};
  double se2_w_temporal_yaw_{12.0};
  double se2_temporal_match_distance_{0.45};
  double se2_w_step_{2.0};
  double se2_max_step_{0.20};
  double se2_max_yaw_delta_{0.45};
  int se2_iterations_{35};
  double se2_learning_rate_{0.04};
  double se2_fd_eps_{0.02};
  int se2_max_optimization_points_{160};
  int se2_post_smooth_passes_{2};
  double se2_post_smooth_alpha_{0.35};
  bool has_last_se2_path_{false};
  bool has_last_goal_{false};
  std::vector<SE2Pose> last_se2_path_;
